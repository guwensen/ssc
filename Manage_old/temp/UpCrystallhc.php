...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	if ($db->query($sql, 2))
	{
		$SumAmount = new SumAmountlhc($arr['s_number'], false, $arr['uid'], $sum);
...
{
	$sql = "SELECT g_id FROM g_zhudan WHERE g_t_id = '{$uid}' AND g_type = '六合彩' AND g_qishu = '{$result[0]['g_qishu']}}' LIMIT 1 ";
	if ($db->query($sql, 0))
		exit(back('很遺憾！'.$uid.'# 注單已有關聯走飛記錄，無法修改。'));
	
	$ps = false;
	if (!$db->query("SELECT g_qishu FROM g_history_lhc WHERE g_qishu = '{$result[0]['g_qishu']}' LIMIT 1 ", 0)){
		if ($db->query("SELECT g_id FROM g_kaipan_lhc WHERE g_qishu = '{$result[0]['g_qishu']}' LIMIT 1", 0))
			$ps = true;
		else
                                ...
                                <tr align="right" style="height:30px">
                                	...
                                	<td align="left">&nbsp;
                                		<select name="s_numcontent">
                                			<?php 	if ($result[0]['g_mingxi_1']=='正碼一' || $result[0]['g_mingxi_1']=='正碼二' || $result[0]['g_mingxi_1']=='正碼三' || $result[0]['g_mingxi_1']=='正碼四' || $result[0]['g_mingxi_1']=='正碼五' || $result[0]['g_mingxi_1']=='正碼六'){ 
															$numarr=array();
															for($i=1;$i<=49;$i++){
											 		...
											 		}else if ($result[0]['g_mingxi_1']=='特碼') { 
														$numarr=array();
														for($i=1;$i<=49;$i++){
															$numarr[$i-1]= strlen($i)==1 ? "0".$i : $i; 
														}
													...
													}else if ($result[0]['g_mingxi_1']=='正碼') { 
														$numarr=array();
														for($i=1;$i<=49;$i++){
															$numarr[$i-1]= strlen($i)==1 ? "0".$i : $i; 
														}
													...
													}else if($result[0]['g_mingxi_1']=='半波'){
														$numarr = array("紅單","紅雙","紅大","紅小","綠單","綠雙","綠大","綠小","藍單","藍雙","藍大","藍小"); 
														foreach($numarr as $str){
															echo "<option value='{$str}' ".($result[0]['g_mingxi_2']==$str ? "selected" : "").">{$str}</option>";	
														}
													}else if($result[0]['g_mingxi_1']=='五行'){
														$numarr = array("金","木","水","火","土"); 
														foreach($numarr as $str){
															echo "<option value='{$str}' ".($result[0]['g_mingxi_2']==$str ? "selected" : "").">{$str}</option>";	
														}
													}else if($result[0]['g_mingxi_1']=='特碼生肖' || $result[0]['g_mingxi_1']=='一肖'){
														$numarr = array_keys($CONFIG['lhc_rgb']['SX']); 
														foreach($numarr as $str){
															echo "<option value='{$str}' ".($result[0]['g_mingxi_2']==$str ? "selected" : "").">{$str}</option>";	
														}
													}else if($result[0]['g_mingxi_1']=='特尾' || $result[0]['g_mingxi_1']=='尾數'){ 
														for($i=0;$i<10;$i++){
															$str = $i; 
															echo "<option value='{$str}' ".($result[0]['g_mingxi_2']==$str ? "selected" : "").">{$str}</option>";	
														}
													}else if($result[0]['g_mingxi_1']=='特碼頭'){ 
														for($i=0;$i<5;$i++){
															$str = $i; 
															echo "<option value='{$str}' ".($result[0]['g_mingxi_2']==$str ? "selected" : "").">{$str}</option>";	
														}
													}else{ 
														 echo "<script>alert('連碼或者合肖不提供修改');history.back();</script>";exit;
													}
