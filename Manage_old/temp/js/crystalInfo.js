var _id = null;
$(function(){
	$.post("/Manage/temp/ajax/numjson.php", {mid : 1}, function(data){
		if (data != null){
			var a=new Array();
...
function FromSubmit($this, sInt, sInt2){
	var href;
	var r = $("#rs").attr("checked") ? 1 : 0;
	if (sInt == 1){
		href = "CrystalInfo.php?uid="+$this.value+"&tid="+sInt2+"&rid="+r;
	} else {
		var Find = $("#"+$this);
		...
		if (searchName.val() == ""){
			...
			href = "CrystalInfo.php?Find="+Find.val()+"&searchName="+searchName.val()+"&rid="+r;
		}
	}
	location.href = href;
}
