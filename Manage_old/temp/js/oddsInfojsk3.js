var _url = "/Manage/temp/ajax/oddsJsonjsk3.php";
var val = 1;
$(function(){
		loadOdds();
});
...
function loadOdds(){
	...
	var a = new Array('a', 'b','c','d',"e","f","g","h","i","j","k","l","m","n","o","p","q","r","s","t");
...
function upOddaAllg($this){
	if (confirm("確認更變賠率碼？")){
		var oddsType = $("#oddsType").val();
		var s_num = $("#s_num").val(); 
		var Ho = $("#Ho").val();
		if (Ho == "" || !/^[0-9]+\.?[0-9]*$/.test(Ho)){$("#Ho").val("0.001");return}
		$.post(_url, {mid : 6, oddsType : oddsType,   s_num : s_num, sHo : Ho}, function(data){
