...
else if (isset($_GET['numSear']))
{
	...
	//搜索出单期的zerc20120803
	$sql = "SELECT g_qishu FROM g_history WHERE g_game_id = 2 and g_qishu='".intval($_GET['numSear'])."' ORDER BY g_qishu DESC ";
...
else 
{
	...
	//取出最近20期未結算注單
	$sql = "SELECT g_qishu FROM g_zhudan WHERE g_win is null AND g_type='重慶時時彩' GROUP BY g_qishu DESC ";
	$Number = $db->query($sql, 1);
	$NumberArr = array();
	if ($Number){
		for ($i=0; $i<count($Number); $i++){
			$sql = "SELECT g_jiner, g_mingxi_1_str FROM g_zhudan WHERE g_qishu ='{$Number[$i]['g_qishu']}' AND g_win is null and g_type='' ";
			$result = $db->query($sql, 1);
			if ($result){
				$m = array('g_id'=>0, 'g_jiner'=>0);
				for ($n=0; $n<count($result); $n++){
