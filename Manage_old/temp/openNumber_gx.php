...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	if (!empty($_POST['s_num1'])&&!empty($_POST['s_num2'])&&!empty($_POST['s_num3'])&&
		  !empty($_POST['s_num4'])&&!empty($_POST['s_num5'])){
			...
			if ($db->query("SELECT g_id FROM g_history3 WHERE g_id = '{$_GET['UpNumcid']}' AND g_game_id =3 LIMIT 1", 0)){
				...
				$db->query($sql, 2);
				exit(alert_href('更變成功。', 'openNumber_gx.php'));
			}
	} else {
		if (!Matchs::isNumber($_POST['number'])) exit(back('開獎期數格式錯誤！'));
		...
		$sql ="SELECT g_id FROM g_history3 WHERE g_qishu = '{$arr['g_qishu']}' AND g_game_id = 3 AND g_ball_1 is not null LIMIT 1";
		if ($db->query($sql, 0)){
			...
			if ($db->query($sql, 2)){
				if ($_POST['cry'] == 1||$_POST['cry'] == 'on'){
					$SumAmount = new SumAmountgx($arr['g_qishu']);
...
else if (isset($_GET['startId']) && $_GET['startId'] == 1)
{
	//執行結算
	$numberId = $_GET['numId'];
	$sql ="SELECT g_id FROM g_history3 WHERE g_qishu = '{$numberId}' AND g_game_id =3 AND g_ball_1 is not null LIMIT 1";
	if ($db->query($sql, 0)){
		$SumAmount = new SumAmountgx($numberId);
...
else if (isset($_GET['UpNumcid']))
{
	...
	$sql = "SELECT * FROM g_history3 WHERE g_id = '{$UpNumcid}' AND g_game_id = 3 LIMIT 1";
	$UpNums = $db->query($sql, 1);
}
else if (isset($_GET['numSear']))
{
	for ($i=0; $i<21; $i++){$arr[$i] = $i+1;}
	//搜索出单期的zerc20120803
	$sql = "SELECT g_qishu FROM g_history WHERE g_game_id = 3 and g_qishu='".intval($_GET['numSear'])."' ORDER BY g_qishu DESC ";
...
else 
{
	for ($i=0; $i<21; $i++){$arr[$i] = $i+1;}
	//取出最近84期未結算注單
	$sql = "SELECT g_qishu FROM g_zhudan WHERE g_win is null AND g_type='廣西快樂十分' GROUP BY g_qishu DESC ";
	$Number = $db->query($sql, 1);
	$NumberArr = array();
	if ($Number){
		for ($i=0; $i<count($Number); $i++){
...
<!--
	...
	function isSumAmount(){
		...
		if (number.value == "" || number.value.length != 9){alert("開獎期數格式錯誤！"+number.value.length);return false;}
		if(cry.checked){
	...
	function numberUp(){
		...
		if(confirm("確定恢復 第 "+num+" 期 注單嗎？")){
			$.post("/ajax/numberUp.php", {mid : 1, num : num, type : 1}, function(data){
				if(data != 1){
                        ...
                        <!-- strat -->
                            <table border="0" cellspacing="0" class="conter">
                                ...
                                <tr style="height:30px" align="center" onmouseover="this.style.backgroundColor='#FFFFA2'" onmouseout="this.style.backgroundColor=''">
                                	...
                                	<?php } else { echo$UpNums[0]['g_qishu'];}?>                                	</td>
                            ...
                            <table border="0" cellspacing="0" class="conter2" style="display:none" id="NumberList">
                            	<tr class="tr_top">
                            		<td width="150">開獎期數</td>
                                    <td>開獎時間</td>
                                    <td colspan="5" width="200">開出號碼</td>
                                    <td colspan="4">總和</td>
                                    <td>龍虎</td>
                                    <td width="120">基本操作</td>
                            	</tr>
                            	<tr>
                            		<td class="left_p6" colspan="10" style="height:30px">
										...
										<table border="0" cellspacing="0" cellpadding="0">
			                                 <tr>
				                                 <td class="nones" width="14" height="18"><img src="/Manage/temp/images/edt.gif"/></td>
				                                  <td class="nones" width="30"><a href="openNumber_gx.php?UpNumcid=<?php echo$numberList[$i][0]?>">修改</a></td>
				                                  <td class="nones" width="15"><img src="/Manage/temp/images/edit.gif"/></td>
				                                  <td class="nones" width="30"><a onmousemove="status=''" href="javascript:if(confirm('確定刪除嗎？')){location.href= 'openNumber_gx.php?Numdelid=<?php echo$numberList[$i][0]?>'}">刪除</a></td>
			                               </tr>
                           				</table>
									</td>
                            	</tr>
                            	<?php }}?>
                            	<tr>
                					<td colspan="13" class="bj"><?php echo $page->fpage(array(0,1, 3,4,5,6,7))?></td>
                				</tr>
                            </table>
                        <!-- end -->
                        </td>
                        ...
                        <td width="15%">
							<table border="0" cellspacing="0" cellpadding="0">
                                 <tr>     
                                    ...
                                    <td class="nones" width="30"><a href="javascript:void(0)" onclick="crySum('openNumber_gx.php?startId=1&numId=<?php echo$NumberArr[$i]['g_qishu']?>')">結算</a></td>
                                 </tr>
                           </table>
						</td>
                    </tr>
