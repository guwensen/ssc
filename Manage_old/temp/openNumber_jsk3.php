...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	if (!empty($_POST['s_num1'])||!empty($_POST['s_num2'])||!empty($_POST['s_num3']) )
	{
		...
		if ($db->query("SELECT g_id FROM g_history9 WHERE g_id = '{$_GET['UpNumcid']}' AND g_game_id =9 LIMIT 1", 0))
		{
			...
			$db->query($sql, 2);
			exit(alert_href('更變成功。', 'openNumber_jsk3.php'));
		}
	}
	else 
	{
		...
		$sql ="SELECT g_id FROM g_history9 WHERE g_qishu = '{$arr['g_qishu']}' AND g_game_id = 2 AND g_ball_1 is not null LIMIT 1";
		if ($db->query($sql, 0))
		{
		...
		{
			$sql = "INSERT INTO g_history9 (g_qishu, g_date, g_game_id, g_ball_1,g_ball_2,g_ball_3 ) VALUES 
			(
				'{$_POST['number']}',
				'{$_POST['openDate']}',
				'9',
			...
			if ($db->query($sql, 2))
			{
				...
				{
					$SumAmountcq = new SumAmountjsk3($arr['g_qishu']);
...
else if (isset($_GET['startId']) && $_GET['startId'] == 1)
{
	...
	if ($db->query($sql, 0)){
		$Amount = new SumAmountjsk3($numberId);
...
else if (isset($_GET['UpNumcid']))
{
	...
	$sql = "SELECT * FROM g_history9 WHERE g_id = '{$UpNumcid}' AND g_game_id = 9 LIMIT 1";
	$UpNums = $db->query($sql, 1);
}
else 
{
	$sql = "SELECT g_qishu FROM g_zhudan WHERE g_win is null AND g_type='江苏骰寶(快3)' GROUP BY g_qishu DESC ";
	...
	if ($Number)
	{
		...
		for ($i=0; $i<$c; $i++)
		{ 
			$result = $db->query("SELECT g_jiner, g_mingxi_1_str FROM g_zhudan WHERE g_win is null AND g_type='江苏骰寶(快3)' AND g_qishu='{$Number[$i][0]}' ", 1);
			if ($result)
			{
	...
	//$r = $db->query("SELECT g_qishu FROM g_zhudan WHERE g_type='廣東快樂十分' AND g_win is not null GROUP BY g_qishu DESC LIMIT 10", 0);
}
?>
...
<!--
	...
	function numberUp(){
		...
		if(confirm("確定恢復 第 "+num+" 期 注單嗎？")){
			$.post("/ajax/numberUp.php", {mid : 1, num : num, type : 9}, function(data){
				if(data != 1){
										...
										<table border="0" cellspacing="0" cellpadding="0">
			                                 <tr>
				                                 <td class="nones" width="14" height="18"><img src="/Manage/temp/images/edt.gif"/></td>
				                                  <td class="nones" width="30"><a href="openNumber_jsk3.php?UpNumcid=<?php echo$numberList[$i][0]?>">修改</a></td>
				                                  <td class="nones" width="15"><img src="/Manage/temp/images/edit.gif"/></td>
				                                  <td class="nones" width="30"><a onmousemove="status=''" href="javascript:if(confirm('確定刪除嗎？')){location.href= 'openNumber_jsk3.php?Numdelid=<?php echo$numberList[$i][0]?>'}">刪除</a></td>
			                               </tr>
                           				</table>
									</td>
                            	</tr>
                        ...
                        <td width="15%">
							<table border="0" cellspacing="0" cellpadding="0">
                                 <tr>     
                                    ...
                                    <td class="nones" width="30"><a href="javascript:void(0)" onclick="crySum('openNumber_jsk3.php?startId=1&numId=<?php echo$NumberArr[$i]['g_qishu']?>')">結算</a></td>
                                 </tr>
                           </table>
						</td>
                    </tr>
