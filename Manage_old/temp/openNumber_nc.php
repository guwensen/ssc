...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	if (!empty($_POST['s_num1'])&&!empty($_POST['s_num2'])&&!empty($_POST['s_num3'])&&
		  ...
		  !empty($_POST['s_num7'])&&!empty($_POST['s_num8'])){
			...
			if ($db->query("SELECT g_id FROM g_history5 WHERE g_id = '{$_GET['UpNumcid']}' AND g_game_id =5 LIMIT 1", 0)){
				$sql = "UPDATE g_history5 SET 
		...
		if (!Matchs::isNumber($_POST['number'],11,11)) exit(back('開獎期數格式錯誤3！'));
		...
		$sql ="SELECT g_id FROM g_history5 WHERE g_qishu = '{$arr['g_qishu']}' AND g_game_id = 5 AND g_ball_1 is not null LIMIT 1";
		if ($db->query($sql, 0)){
			exit(back('第 '.$arr['g_qishu'].' 已經存在！'));
		} else {
			$sql = "INSERT INTO g_history5 (g_qishu, g_date, g_game_id, g_ball_1,g_ball_2,g_ball_3,g_ball_4,g_ball_5,g_ball_6,g_ball_7,g_ball_8) VALUES 
			(
				'{$arr['g_qishu']}',
				'{$arr['g_date']}',
				'5',
			...
			if ($db->query($sql, 2)){
				if ($_POST['cry'] == 1||$_POST['cry'] == 'on'){
					$SumAmount = new SumAmountnc($arr['g_qishu']);
...
else if (isset($_GET['startId']) && $_GET['startId'] == 1)
{
	//執行結算
	$numberId = $_GET['numId'];
	$sql ="SELECT g_id FROM g_history5 WHERE g_qishu = '{$numberId}' AND g_game_id =5 AND g_ball_1 is not null LIMIT 1";
	if ($db->query($sql, 0)){
		$SumAmount = new SumAmountnc($numberId);
...
else if (isset($_GET['numDelid']) || isset($_GET['Numdelid']))
{
	if (isset($_GET['numDelid'])){
		...
		$sql = "DELETE FROM g_history5 WHERE g_date < '{$numDelid}' ";
	} else {
		$numDelid = $_GET['Numdelid'];
		$sql = "DELETE FROM g_history5 WHERE g_id = '{$numDelid}' ";
	}
	$db->query($sql, 2);
	exit(back('刪除成功。'));
}
else if (isset($_GET['UpNumcid']))
{
	...
	$sql = "SELECT * FROM g_history5 WHERE g_id = '{$UpNumcid}' AND g_game_id = 5 LIMIT 1";
...
else 
{
	...
	//取出最近84期未結算注單
	$sql = "SELECT g_qishu FROM g_zhudan WHERE g_win is null AND g_type='幸运农场' GROUP BY g_qishu DESC ";
...
<!--
	...
	function numberUp(){
		...
		if(confirm("確定恢復 第 "+num+" 期 注單嗎？")){
			$.post("/ajax/numberUp.php", {mid : 1, num : num, type : 5}, function(data){
				if(data != 1){
                            ...
                            <table border="0" cellspacing="0" class="conter">
                                ...
                                <tr style="height:30px" align="center" onmouseover="this.style.backgroundColor='#FFFFA2'" onmouseout="this.style.backgroundColor=''">
                                    ...
                                    <?php if (!$UpNumcid){?>
	                                    ...
	                                    <?php }else {?>
	                                    <input type="text" name="s_num6" class="texta" value="<?php echo$UpNums[0]['g_ball_6']?>" />
	                                    <?php }?>
	                                </td>
                                    <td>
                                    <?php if (!$UpNumcid){?>
	                                    ...
	                                    <?php }else {?>
	                                    <input type="text" name="s_num7" class="texta" value="<?php echo$UpNums[0]['g_ball_7']?>" />
	                                    <?php }?>
	                                </td>
                                    <td>
                                    <?php if (!$UpNumcid){?>
	                                    ...
	                                    <?php }else {?>
	                                    <input type="text" name="s_num8" class="texta" value="<?php echo$UpNums[0]['g_ball_8']?>" />
	                                    <?php }?>
	                                </td>
                                </tr>
                            </table>
                            <table border="0" cellspacing="0" class="conter" style="display:none" id="NumberList">
                            	<tr class="tr_top">
                            		<td width="130">開獎期數</td>
                                    <td width="100">開獎時間</td>
                                    <td width="700">開出號碼</td>
                                    <td colspan="4">總和</td>
                                    <td>家禽野兽</td>
                            	...
                            	<?php }}?>
                            	<tr>
                					<td colspan="10" class="bj"><?php echo $page->fpage(array(0,1, 3,4,5,6,7))?></td>
                				</tr>
                            </table>
                        <!-- end -->
                        </td>
