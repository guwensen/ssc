...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	if (!empty($_POST['s_num1'])&&!empty($_POST['s_num2'])&&!empty($_POST['s_num3'])&&
		...
		if ($db->query($sql, 0)){
			...
			$sql = "INSERT INTO g_history6 (g_qishu, g_date, g_game_id, g_ball_1,g_ball_2,g_ball_3,g_ball_4,g_ball_5,g_ball_6,g_ball_7,g_ball_8) VALUES 
			(
				'{$arr['g_qishu']}',
				'{$arr['g_date']}',
				'6',
...
else if (isset($_GET['numSear']))
{
	...
	//搜索出单期的zerc20120803
	$sql = "SELECT g_qishu FROM g_history6 WHERE g_game_id = 6 and g_qishu='".intval($_GET['numSear'])."' ORDER BY g_qishu DESC ";
...
else 
{
	for ($i=0; $i<10; $i++){$arr[$i] = $i+1;}
	//取出最近179期未結算注單
	$sql = "SELECT g_qishu FROM g_zhudan WHERE g_win is null AND g_type='北京赛车PK10' GROUP BY g_qishu DESC ";
	$Number = $db->query($sql, 1);
	$NumberArr = array();
	if ($Number){
		for ($i=0; $i<count($Number); $i++){
...
<!--
	...
	function numberUp(){
		...
		if(confirm("確定恢復 第 "+num+" 期 注單嗎？")){
			$.post("/ajax/numberUp.php", {mid : 1, num : num, type : 6}, function(data){
				if(data != 1){
                            ...
                            <table border="0" cellspacing="0" class="conter">
                                ...
                                <tr style="height:30px" align="center" onmouseover="this.style.backgroundColor='#FFFFA2'" onmouseout="this.style.backgroundColor=''">
                                    ...
                                    <?php if (!$UpNumcid){?>
	                                    ...
	                                    <?php }else {?>
	                                    <input type="text" name="s_num9" class="texta" value="<?php echo$UpNums[0]['g_ball_9']?>" />
	                                    <?php }?>
	                                </td>
									 <td>
                                    <?php if (!$UpNumcid){?>
	                                    ...
	                                    <?php }else {?>
	                                    <input type="text" name="s_num10" class="texta" value="<?php echo$UpNums[0]['g_ball_10']?>" />
	                                    <?php }?>
	                                </td>
                                </tr>
                            </table>
                            <table border="0" cellspacing="0" class="conter1" style="display:none" id="NumberList">
                            	<tr class="tr_top">
                            		<td width="180">開獎期數</td>
                                    <td width="200">開獎時間</td>
                                    <td width="400" colspan="10">開出號碼</td>
                                    <td colspan="3">冠亞軍和</td>
                                    <td colspan="5">1～5 龍虎</td>
                                    <td width="120">基本操作</td>
                            	</tr>
                            	<tr>
                            		<td class="left_p6" colspan="21" style="height:30px">
                            	...
                            	<?php }}?>
                            	<tr>
                					<td colspan="21" class="bj"><?php echo $page->fpage(array(0,1, 3,4,5,6,7))?></td>
                				</tr>
                            </table>
                        <!-- end -->
                        </td>
