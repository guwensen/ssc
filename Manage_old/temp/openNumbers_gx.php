...
if (isset($_GET['delid']) && Matchs::isNumber($_GET['delid']))
{
	...
	if ($id)
	{
		...
		exit(alert_href('刪除成功', 'openNumbers_gx.php'));
	}
	else 
	{
...
if (isset($_GET['inserid']) && Matchs::isNumber($_GET['inserid']))
{
	...
	InsertNumbergx($inserid, $ConfigModel['g_close_time']);
	exit(alert_href('操作成功', 'NumberInclude.php'));
}
$result=mysql_query("select * from g_kaipan3  order by g_qishu desc LIMIT 1");
          ...
          <td class="c"><!-- strat -->
            <table border="0" cellspacing="0" class="conter">
             <tr class="tr_top">
              ...
              <?	
		 $result = mysql_query("SELECT `g_id`, `g_qishu`, `g_feng_date`, `g_open_date`, `g_kai_date`, `g_lock` FROM `g_kaipan3` ORDER BY g_qishu ASC "); 
...
?>
        ...
        <tr>
          ...
          <td class="f" align="center"><span class="odds">（注意：系統會自動加載1-50期號碼）</span> 天:
