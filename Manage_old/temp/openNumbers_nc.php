...
{
	$delid = $_GET['delid'];
	$id = $db->query("SELECT g_lock FROM g_kaipan5 WHERE g_id = '{$delid}' LIMIT 1 ", 0);
	if ($id)
	{
		$db->query("DELETE FROM g_kaipan5 WHERE g_id = '{$delid}' LIMIT 1", 2);
...
{
	$openid = $_GET['openid'];
	$openids = $db->query("SELECT g_lock FROM g_kaipan5 WHERE g_qishu = '{$openid}' LIMIT 1 ", 0);
	if ($openids)
	{
		$db->query("DELETE FROM g_kaipan5 WHERE g_qishu < '{$openid}' ", 2);
		$db->query("UPDATE g_kaipan5 SET g_lock = 2 WHERE g_qishu = '{$openid}' LIMIT 1 ", 2);
...
if (isset($_GET['inserid']) && Matchs::isNumber($_GET['inserid']))
{
	...
	insertNumbernc('09:53:00', $inserid, 10, 14, 110, $ConfigModel['g_close_time']);
	exit(alert_href('操作成功', 'NumberInclude.php'));
}
else 
{
	$result = $db->query("SELECT `g_id`, `g_qishu`, `g_feng_date`, `g_open_date`, `g_lock` FROM `g_kaipan5` ORDER BY g_qishu ASC ", 1);
}
?>
            ...
            <td class="c">
            	<table border="0" cellspacing="0" class="main">
                    ...
                    <tr>
                        ...
                        <span class="odds">（注意：系統會自動加載1-97期號碼）</span>
