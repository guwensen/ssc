...
if (isset($_GET['delid']) && Matchs::isNumber($_GET['delid']))
{
	...
	if ($id)
	{
		...
		exit(alert_href('刪除成功', 'openNumbers_xj.php'));
	}
	else 
	{
...
if (isset($_GET['openid']) && Matchs::isNumber($_GET['openid'], 10, 11))
{
	$openid = $_GET['openid'];
...
if (isset($_GET['inserid']) && Matchs::isNumber($_GET['inserid']))
{
	...
	insertNumberxj($inserid,$ConfigModel['g_close_time']);
	exit(alert_href('操作成功', 'NumberInclude.php'));
}
else 
{
	$result = $db->query("SELECT `g_id`, `g_qishu`, `g_feng_date`, `g_open_date`, `g_lock` FROM `g_kaipan8` ORDER BY g_qishu ASC ", 1);
}
?>
            ...
            <td class="c">
            	<table border="0" cellspacing="0" class="main">
                    ...
                    <tr>
                        ...
                        <span class="odds">（注意：系統會自動加載1-120期號碼）</span>
